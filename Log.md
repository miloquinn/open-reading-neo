...
- 将首次启动页面从“介绍 + 完整条款 + 隐私 + 两项勾选”同屏布局拆为“软件介绍 → 使用协议 → 隐私说明”三步；完整法律文本仍保留，第三方书源责任边界继续单独确认，最后一步隐私确认后才持久化协议完成状态。
- 步骤切换使用书页方向的淡入、横向轻移、缩放和步骤完成勾选动效；首屏用书本轨道视觉表达本地阅读，最终进入 App 前做轻微淡出。手机端文档继续独立滚动、底部确认区固定；按钮在窄屏自动收缩文案，避免横向溢出。
