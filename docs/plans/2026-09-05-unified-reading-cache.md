...
- Validation includes cache/storage/DAO/codec tests and local/online reader regressions. Full analysis reports only the pre-existing unused import in `test/source_rule_xpath_test.dart`; this unrelated concurrently authored file is preserved. No physical-device performance benchmark has been run.
