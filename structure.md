...
- `pages/reader/native_reader_page.dart`：本地 TXT、EPUB 等内容适配器；全局阅读字体尚未从磁盘恢复并完成运行时注册时只显示主题化打开占位。正文排版读取共享的字重、字间距与对齐方式，重排后按 canonical 文本锚点恢复位置。EPUB 的 NCX/Navigation Document 目录链接会保留 fragment，并映射到解析后正文的 UTF-16 offset；同一 XHTML 内的二级标题可精确跳转，活动小节按全书目录目标位置判定，章节内容跨越相邻 XHTML 时仍保持上一小节为当前状态，直到下一个锚点进入阅读位置。
...
- `OnlineFontService` 将下载进度保留在独立、节流的局部监听器中，网络分块不再触发 `AppSettingsNotifier` 全局重建；大字体签名检查与 SHA-256 流式校验在后台 isolate 完成，UI isolate 只承担最终 `FontLoader` 注册。官方字体只提供 ZIP 总包时，可用受限 HTTP Range 读取指定 Deflate 条目，解压后必须通过大小、字体签名和固定 SHA-256 校验，服务端不返回 `206` 时拒绝下载，避免误取整个压缩包。
