...
void main() {
  ...
  testWidgets('active chrome can pause, resume, and stop auto page turn', (
    ...
    final fade = tester.widget<FadeTransition>(
      find.ancestor(of: bar, matching: find.byType(FadeTransition)).first,
    );
  ...
  testWidgets('shortcut follows chrome with animated entry and exit', (
    ...
    final fade = find.descendant(
      of: find.byKey(
        const ValueKey('reader-auto-page-turn-shortcut-visibility'),
      ),
    ...
    final partial = opacity();
    visible.value = false;
    await tester.pump();
    expect(opacity(), closeTo(partial, 0.001));
    await tester.pump(const Duration(milliseconds: 80));
    visible.value = true;
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 350));
