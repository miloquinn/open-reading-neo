...
void main() {
  ...
  for (final chapterCount in [24, 101, 654]) {
    test('loads all $chapterCount chapters through the original XPath rules', () async {
      ...
      final chapters = await runtime.getChapters(source, 'https://books.test/book/');

      expect(chapters, hasLength(chapterCount));
      expect(chapters.map((chapter) => chapter.order), List.generate(chapterCount, (i) => i));
      expect(chapters.map((chapter) => chapter.title), List.generate(chapterCount, (i) => 'Chapter ${i + 1}'));
      expect(chapters.map((chapter) => chapter.id), List.generate(chapterCount, (i) => 'https://books.test/book/read_${i + 1}.html'));
...
class _CatalogTransport implements SourceTransport {
  ...
  String _entries(List<int> chapters) => chapters.map((chapter) =>
