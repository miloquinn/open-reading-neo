// flutter test tool/preview_reader_aloud_upgrade.dart
import 'dart:io';
...
void main() {
  testWidgets('capture listening controls, settings and named voices', (
    tester,
  ) async {
    // This preview runs under flutter test and never reads real preferences.
    ...
    for (final scenario in [
      ...
      await aloud.setPresentation(ReaderAloudPresentation.controls);
      await aloud.saveCloudProfile(
      ...
      await aloud.selectCloudProfile('narrator');
          ...
          home: Scaffold(
            body: SafeArea(
              child: Padding(
                ...
                child: Text(
                  '珍藏版序言\n\n夜色渐深，窗外的风轻轻翻过书页。阅读让我们在平凡的日子里，发现更辽阔的世界。\n\n每一个故事，都值得慢慢听。',
                  style: TextStyle(
                    fontSize: 20,
                    height: 1.9,
                    color: palette.text,
      ...
      final readerContext = tester.element(find.byType(Scaffold));
      showReaderAloud(
      ...
      Future<void> capture(String suffix) async {
        ...
        await tester.runAsync(() async {
          ...
          final file = File(
            '.omx/aloud-upgrade-previews/${scenario.name}-$suffix.png',
          );
      ...
      final engineButton = find.byKey(const ValueKey('reader-aloud-engine'));
      await tester.ensureVisible(engineButton);
      await tester.tap(engineButton);
      await tester.pumpAndSettle();
      await capture('settings');
      Navigator.of(tester.element(find.byType(ReaderAloudPanel))).pop();
      await tester.pumpAndSettle();
      Navigator.of(readerContext).push(
        MaterialPageRoute<void>(
          builder: (_) => CloudTtsSettingsPage(service: aloud),
        ),
      );
      await tester.pumpAndSettle();
